/**
 * Load one race's published artifacts (chains, entities, donors, ads, ledger, trails) into Neo4j (D-83). Idempotent: nodes and edges are MERGEd on their keys, so re-running
    ...
    for (const rows of batches(nodes)) {
      await run(
        `UNWIND $rows AS n
         ...
         SET e.name = n.name, e.title = n.title, e.name_lc = n.name_lc, e.kind = n.kind, e.committee_type = n.committee_type,
