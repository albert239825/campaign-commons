...
import { Chip, Legend, SourceLink, Swatch } from "@/components/ui";
import { AD_TYPE_LABEL, Creative, IssueChips, SameWindowBuys, VendorLines, isVerified } from "@/components/ads/ad-card";
import { adFocusWire, spendMidpoint } from "@/components/chain/ad-view";
    ...
    <div className="ad-page detail-page">
      ...
      <section className="ad-page-section space-y-3">
        ...
        <p className="text-xs text-neutral-500">
          Creative cached from the platform&apos;s transparency library. <SourceLink href={ad.creative_url} label="live ad library record" />
        </p>
      </section>
      ...
      <section className="ad-page-section space-y-3">
        ...
        <p className="text-sm">
          {AD_TYPE_LABEL[ad.ad_type]} ad run by {ad.advertiser_name} on {platform} from {date(ad.first_shown)} to {date(ad.last_shown)}
          {ad.regions.length > 0 ? ` in ${ad.regions.join(", ")}` : ""}. Spend {range(ad.spend_range.min, ad.spend_range.max, (n) => money(n))} (platform-reported
          range, not a filed figure; midpoint {money(spendMidpoint(ad.spend_range), { compact: false })}); impressions {range(ad.impressions_range.min, ad.impressions_range.max)}.{" "}
      ...
      <section className="ad-page-section space-y-3">
        ...
        {dark !== null && sponsorId && (
          ...
          >
            ...
            {fecEvidenceUrls.length > 0 && <SourceLink href={fecEvidenceUrls[0]} label="fec.gov record" />}
          </p>
        )}
        {ad.issues && (
      ...
      {(chain || sponsorId) && (
        <section className="ad-page-section space-y-3">
          <h2>Where the sponsor&apos;s money came from, and how it reached this ad</h2>
          {chain ? (
            <>
              <Legend
                items={[
      ...
      <section className="ad-page-section space-y-3">
        <h2>Which vendor placed this ad</h2>
