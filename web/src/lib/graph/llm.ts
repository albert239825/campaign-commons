...
- Ads: questions about ads, attack ads, negative ads or advertising for/against a candidate are answered with the money that paid for them — TARGETED edges (support_oppose 'O' for negative/attack/against, 'S' for positive/for) from the spender to the candidate, optionally with the spender's PAID edges to vendors. Ad nodes carry no dollars; never give up because of that.
...
- Only return an empty query when the question is not about this race's money at all (weather, biography, policy positions); every question about who paid, spent, gave, funded, supported or opposed is answerable.`;
