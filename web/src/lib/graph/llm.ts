/**
 * The two model calls of graph mode (D-83), both server-only and both constrained.
 *
...
export function checkNarrative(text: string, facts: readonly GraphFact[]): NarrativeCheck {
  ...
  if (cited.length === 0 || cited.some((n) => n < 1 || n > facts.length)) return { ok: false, reason: "bad_citation" };
  ...
  for (const s of sentences) {
    ...
    for (const raw of numbers) {
      ...
      if (!allowed.some((a) => Math.abs(a - v) <= Math.max(0.005 * Math.abs(a), 0.5))) return { ok: false, reason: "unknown_number" };
    }
  }
  return { ok: true };
}
