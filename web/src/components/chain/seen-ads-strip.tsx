      ...
      {ads.map((ad) => (
        ...
        >
          {ad.cached_creative_path ? (
            ...
            <img
              src={ad.cached_creative_path}
              alt={`${ad.advertiser_name} ${ad.ad_type} ad`}
              className="h-12 w-20 object-cover"
              loading="lazy"
            />
          ) : (
            <span className="flex h-12 w-20 items-center justify-center bg-neutral-100 text-[10px] uppercase text-neutral-500">
              {ad.ad_type}
            </span>
          )}
          <span className="text-xs">
            <Chip tone="green">verified by hand</Chip>
            <span className="mt-1 block tabular-nums text-neutral-600">
              {range(ad.impressions_range.min, ad.impressions_range.max)}{" "}
              impressions
            </span>
          </span>
        </Link>
