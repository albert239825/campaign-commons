...
const byHopThenAmount = (a: ChainEdge, b: ChainEdge) => a.depth - b.depth || b.amount - a.amount;
 ...
 */
export function EdgeTable({ chain, links }: { chain: Chain; links: NodeLinks }) {
  const byAmount = chain.edges.slice().sort((a, b) => b.amount - a.amount);
  const head = fromWireRows(toWireRows(byAmount.slice(0, TOP_EDGES).sort(byHopThenAmount), chain, links));
  const tail = toWireRows(byAmount.slice(TOP_EDGES).sort(byHopThenAmount), chain, links);
  return (
    <div className="space-y-3">
      <EdgeRows rows={head} />
      {tail.rows.length > 0 && <EdgeTail wire={tail} total={chain.edges.length} />}
    </div>
  );
}
