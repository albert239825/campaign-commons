...
| Q-12 | Vendor→ad: we ship adjacent (date window) everywhere, inferred (single digital vendor in window) where it holds, verified only with a source. Is the adjacent copy — "in that window the sponsor paid X and Y for digital buys; FEC does not record which buy placed which ad" — clear enough for a first-time reader? | Test on Patrick / a non-team reader during the Block 2 preview. | Albert |
| Q-13 | Issue tagging is by hand (top ~40 ads, top ~50 IE notices, top 20 spenders). Do we want an LLM-assisted first pass with human confirmation, or stay LLM-free (current stance, D-09)? | Coverage numbers in `issues.json.coverage` will show how far hand tagging gets. | Albert |
