...
| `adjacent` (**dropped in D-74** — date overlap is now `Ad.same_window_buys[]` context, never an edge) | ~~dotted~~ | "Ran Oct 1–14. In that window the sponsor paid X and Y for digital buys; FEC does not record which buy placed which ad." | co-occurrence only |
